...
from indicies import PX, PY, PZ, TA, TB, TC
...
class STLCanvas:
    ...
    def add_shape(self, shape):
        self.triangles.extend(shape.get_triangles())

    def compute_area(self):
        area = 0.0
        for tri in self.triangles:
            area += .5 * \
                norm(np.cross(np.subtract(
                    tri[TB], tri[TA]), np.subtract(tri[TB], tri[TC])))
        return area
    ...
    def write_stl(self, outfile, make_positive=True):
        ...
        with open(outfile, 'wb') as dest_file:
            dest_file.write(struct.pack(
                "80sI", b'Quick Release Lever', len(self.triangles)))
            for tri in self.triangles:
